...
{	
	...
	public function beforeSaveDB()
	{
		...
		$sql.= ", cap_tipo_tramite='$cap_tipo_tramite', cap_observaciones='$cap_observaciones', cap_date=now(), cap_rol_play_aprobado='$cap_rol_play_aprobado' where cap_code = ".intval($cap_code);
			...
			foreach($obj->m_childs['cap_calls'] as $aud)
			{
				...
				if (($aud->m_op == 'A')	|| ($aud->m_op == 'M') || ($aud->m_op == 'B') )
				{
					if (($aud->m_op == 'A')	|| ($aud->m_op == 'M') {
						...
						$cap_call = $aud->getField("cap_call_date")->getValue();
					...
					if ($aud->m_op == 'A')	
					{
						...
						$sql = "INSERT INTO cap_calls(cap_code,cap_call, cap_call_date, cap_call_reference, doc_storage, cap_call_aprobo) ";
						$sql.= "VALUES (".$cap_code.", ".$cap_call.",STR_TO_DATE('".$cap_call_date."','%d/%m/%Y'), '".$cap_call_reference."', '".$doc_storage."', '".$cap_call_aprobo."')";
					}
					elseif ($aud->m_op == 'M')
						$sql = "UPDATE cap_calls SET cap_call_date= STR_TO_DATE('".$cap_call_date."','%d/%m/%Y'), cap_call_reference='".$cap_call_reference."', doc_storage='".$doc_storage."', cap_call_aprobo='".$cap_call_aprobo."' WHERE cap_call=".$cap_call;
					elseif ($aud->m_op == 'B')				
