...
class cdes_circuito extends cobjbase {
    ...
    function __construct() {
        ...
        $this->m_objfactory_sql = "SELECT cir_code,     sum(IF (mon_status='PENDIENTE' , 1 , 0)) as cirg_cant_mon_pendientes,      sum(IF (mon_status='REALIZADO' , 1 , 0)) as cirg_cant_mon_realizados,      sum(IF (mon_status='CERRADO' , 1 , 0)) as cirg_cant_mon_cierre_forz,         sum(IF ((mon_status='REALIZADO' and mon_aprobo='SI') , 1 , 0)) as cirg_cant_mon_ok,      sum(IF ((mon_status='REALIZADO' and mon_aprobo='NO') , 1 , 0)) as cirg_cant_mon_mal,      ifnull((select count(distinct cap_code) from capacitacion c where c.cir_code=m.cir_code and  cap_status='PENDIENTE'),0)  as cirg_cant_cap_pendientes,      ifnull((select count(distinct cap_code) from capacitacion c where c.cir_code=m.cir_code and  cap_status='REALIZADO'),0) as cirg_cant_cap_realizados,      ifnull((select count(distinct cap_code) from capacitacion c where c.cir_code=m.cir_code and  cap_status='REALIZADO' and cap_rol_play_aprobado='SI'),0) as cirg_cant_cap_ok,     ifnull((select count(distinct cap_code) from capacitacion c where c.cir_code=m.cir_code and  cap_status='REALIZADO' and cap_rol_play_aprobado='NO'),0) as cirg_cant_cap_mal,     round( ( sum(IF (mon_status='REALIZADO' ,ifnull(mon_puntaje,0),0)) / sum(IF (mon_status='REALIZADO' , 1 , 0)) ) ,2) as cirg_puntaje_prom     FROM monitoreos m  where 1 = 1 ";
