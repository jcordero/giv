...
class mon_items extends cobjbase {
    ...
    function __construct() {
        ...
        $this->m_fields['it_puntaje'] = new CField(Array("Name"=>"it_puntaje", "Type"=>"int", "IsForDB"=>true, "Order"=>106));
        $this->m_fields['it_aprobo'] = new CField(Array("Name"=>"it_aprobo", "Size"=>2, "IsForDB"=>true, "Order"=>107));
        $this->m_fields['it_perjuicio_cliente'] = new CField(Array("Name"=>"it_perjuicio_cliente", "Size"=>2, "IsForDB"=>true, "Order"=>108));
        $this->m_fields['it_note'] = new CField(Array("Name"=>"it_note", "Size"=>200, "IsForDB"=>true, "Order"=>109));

        //--Contenedores de Clases dependientes
        // No hay clases dependientes

        //Consultas particulares a la base de datos
        $this->m_loaddb_sql = "SELECT mon_code, it_code, it_name, it_order, it_importance, it_puntaje, it_aprobo, it_perjuicio_cliente, it_note FROM mon_items  WHERE mon_code= :mon_code_key: AND it_code= :it_code_key:";
        $this->m_objfactory_sql = "SELECT mon_code, it_code, it_name, it_order, it_importance, it_puntaje, it_aprobo, it_perjuicio_cliente, it_note FROM mon_items";
        $this->m_objfactory_suffix_sql = " ORDER BY it_order";
        $this->m_savedb_update_sql = "UPDATE mon_items SET mon_code= :mon_code:, it_code= :it_code:, it_name= :it_name:, it_order= :it_order:, it_importance= :it_importance:, it_puntaje= :it_puntaje:, it_aprobo= :it_aprobo:, it_perjuicio_cliente= :it_perjuicio_cliente:, it_note= :it_note: WHERE mon_code=:mon_code_key: AND it_code=:it_code_key:";
        $this->m_savedb_insert_sql = "INSERT INTO mon_items(mon_code, it_code, it_name, it_order, it_importance, it_puntaje, it_aprobo, it_perjuicio_cliente, it_note) VALUES (:mon_code:, :it_code:, :it_name:, :it_order:, :it_importance:, :it_puntaje:, :it_aprobo:, :it_perjuicio_cliente:, :it_note:)";
